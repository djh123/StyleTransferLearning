...
class Options():
    def __init__(self):
        ...
        test_arg = subparsers.add_parser("test", help="parser for evaluation/stylizing arguments")
        test_arg.add_argument("--style-folder", type=str, default="images/9styles/",
                                help="path to style-folder")
        test_arg.add_argument("--style-size", type=int, default=512,
                                help="size of style-image, default is the original size of style image")
        test_arg.add_argument("--cuda", type=int, default=1, 
                                help="set it to 1 for running on GPU, 0 for CPU")
        test_arg.add_argument("--content-image", type=str, default="images/content/venice-boat.jpg",
                                help="path to content image you want to stylize")
        test_arg.add_argument("--model", type=str, required=True,
                                help="saved model to be used for stylizing the image")
        test_arg.add_argument("--ngf", type=int, default=128,
                                help="number of generator filter channels, default 128")


        # self.parser.parse_args(train_arg) 

    def parse(self):
        return self.parser.parse_args()
        # return self.parser.parse_args(['train'])
